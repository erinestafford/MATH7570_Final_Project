...
% rhs
n=7
alpha = u_1(0); % boundary condition
beta = u_1(1); % boundary condition
rhs = -2.*ones(length(2:n),1); % exact rho at the grid points
rhs(1)= -2 + alpha/h^2; % add boundary term
rhs(n-1) = -2 + beta/h^2; % add boundary term
